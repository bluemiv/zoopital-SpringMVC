...
<script type="text/javascript">
 	...
 	$(function(){
		...
		$("#start_select").click(function(){
			...
			start_select//끝시간 셀렉트박스&시작시간 다시선택 버튼 나타내고 시작시간 셀렉박스 비활성화
			$("#end_time").show();
	 		$("#start_reselect").show();
			$("#start_time option").attr("readonly", true); 
 		});
		
		//시작 시간 다시선택 버튼 클릭하면
		$("#start_reselect").click(function(){
			//시작시간 셀렉박스 활성화&선택버튼 나타내고 다시선택버튼/끝시간 셀렉박스 또 없애기
			$("#start_time option").attr("readonly", false);
			$("#start_select").show();
			$("#end_time").hide();
			$("#start_reselect").hide();
		});		
...
<form action="reserveInsertPro.pet?start_time=${start_time }&end_time=${end_time}" method="get">
<fieldset>
	<legend>시간 선택</legend>
	...
	<c:set var="start_time" value='${ reserve_start_time }' />	
	</select>
	...
	<c:set var="end_time" value='${ reserve_end_time }' />
	</select>
